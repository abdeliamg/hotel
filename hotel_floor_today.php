...
$stmt = $pdo->query("WITH
-- Rooms active today (supports duplicated rows across different date ranges)
ActiveRooms AS (
    SELECT r.hotel_name, r.floor, r.room_num, r.room_type
    FROM room r
    WHERE DATE('now') BETWEEN r.date_from AND r.date_to
),

-- Counts per hotel/floor for today's active rooms
RoomCounts AS (
    SELECT hotel_name,
           floor,
           COUNT(DISTINCT room_num) AS total_rooms,
...
-- Reserved rooms/beds today per hotel/floor (only for rooms that are active today)
ReservedRooms AS (
    SELECT ar.hotel_name,
           ar.floor,
           COUNT(ar.room_num) AS reserved_rooms,
           SUM(ar.room_type)  AS reserved_beds
    FROM ActiveRooms ar
    JOIN ActiveReservations rs
      ON rs.hotel_name = ar.hotel_name
     AND rs.room_num   = ar.room_num
    GROUP BY ar.hotel_name, ar.floor
)

SELECT rc.hotel_name,
       rc.floor,
...
LEFT JOIN ReservedRooms rr
  ON rc.hotel_name = rr.hotel_name
 AND rc.floor      = rr.floor
ORDER BY rc.hotel_name, rc.floor;
